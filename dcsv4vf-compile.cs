...
static IMyTerminalBlock fighterFcsr = null;
...
Vector3D nowSpeedToMe = Vector3D.TransformNormal(msc.GetShipVelocities().LinearVelocity, refLookAtMatrix);
if (isDown) {
var ds = -1.5;
...
public List<float> pla = new List<float>();
...
,float ps0=0, float pe0=0, float ps1=0, float pe1=0, float ps2=0, float pe2=0, float ns0=0, float ne0=0, float ns1=0, float ne1=0, float ns2=0, float ne2=0, bool tr = false, int wta = -1, bool cfcsr = false) {
pl[0] = p0;
pl[1] = p1;
pl[2] = p2;
delay = d;
...
sTime = t;
tarMode = mode;
if(cf && tarMode < lastMode) callComputer(fighterFcsr, "VFTM" + tarMode);
}
...
var ta = pl[nm];
if (tarMode > 0 && pla.Count>tarMode-1) { 
  if(t > sTime + DELAY_ALL) { ta = pla[tarMode-1];}
}
var need = 0F;
if (isVTA && legMode!=3 ) {
if (VTA >= 0) {
...
, float ps0=0, float pe0=0, float ns0=0, float ne0=0, float ps1=0, float pe1=0, float ns1=0, float ne1=0, float ps2=0, float pe2=0, float ns2=0, float ne2=0, bool tr = false, int wta = -1, bool cf=false
) {
...
foreach(var t in tmprs) li.Add(new VFTRotor((IMyMotorStator)t, p0, p1, p2, d, ps0, pe0, ps1, pe1, ps2, pe2, ns0, ne0, ns1, ne1, ns2, ne2, tr, wta,cf));
...
msc = Head;
upThrusters = forwardThrustersR;
downThrusters = backwardThrustersR;
forwardThrusters = downThrustersR;
backwardThrusters = upThrustersR;
} else {
msc = Cockpit;
...
if(lastMsc!=msc){ lastMsc = msc;resetThrusters();}
...
static void PlayAction(IMyTerminalBlock block, String action, List<TerminalActionParameter> args = null) {
if (block != null) {
...
tryAddVftr("Leg-L2", toRa(-88F), toRa(-88F), toRa(88), 90, vftrList, 0,0,0,0, 0,0,0,0, toRa(0), toRa(70), 0, 0, true, 2, true);
tryAddVftr("Leg-L3", toRa(88F), toRa(-2F), toRa(-88F), 0, vftrList, 0,0,0,0, toRa(0), toRa(70), toRa(0), toRa(-70), 0, 0, 0, toRa(-70), true, 4);
tryAddVftr("Leg-R2", toRa(-88F), toRa(-88F), toRa(88), 90, vftrList, 0,0,0,0, 0,0,0,0, toRa(0), toRa(70), 0, 0, true, 3);
tryAddVftr("Leg-R3", toRa(88F), toRa(-2F), toRa(-88F), 0, vftrList, 0,0,0,0, toRa(0), toRa(70), toRa(0), toRa(-70), 0, 0, 0, toRa(-70), true, 5);

tryAddVftr("Arm-L1", toRa(-20), toRa(-65), toRa(-75), 0, vftrList);
vftrList.Last().pla.Add(toRa(45));
tryAddVftr("Arm-L2", toRa(90), toRa(60), toRa(88F), 180, vftrList);
tryAddVftr("Arm-R1", toRa(20), toRa(65), toRa(75), 0, vftrList);
vftrList.Last().pla.Add(toRa(-45));
tryAddVftr("Arm-R2", toRa(90), toRa(60), toRa(88F), 180, vftrList);
tryAddVftr("Hinge waist", toRa(90), toRa(90), toRa(-89), 360, vftrList);

var tbl = getBlockListByName(DCSLCDNameTag);
LCD = tbl.Select(b => { if (b is IMyTextSurfaceProvider && ((IMyTextSurfaceProvider)b).SurfaceCount>=3 ) { return ((IMyTextSurfaceProvider)b).GetSurface(2); } else return (IMyTextSurface)b; }).ToList();
if(LCD.Count < 1)
{debugInit = DCSLCDNameTag  + " not found";}
...
static void callComputer(IMyTerminalBlock computer, string cmd) {
...
List<IMyThrust> noUse = new List<IMyThrust>();
foreach(var otl in offGridThrustLL) { 
noUse.AddList(otl);
}
for(int i = 0; i < 3; i ++) {
bool iH = i >0;
...
List<IMyThrust> ctl = new List<IMyThrust>();
...
if (scale > 0.8 || (!iH && scale>0.1) )
{
thrustMax += thisThrust.MaxEffectiveThrust * scale;
ctl.Add(thisThrust);
noUse.Remove(thisThrust);
}
...
foreach (IMyThrust thisThrust in ctl)
{
var thrustDirection = thisThrust.WorldMatrix.Forward;
float scale = -(float)thrustDirection.Dot(thrustNeedDir);
if (scale > 0.1)
...
foreach(var n in noUse) { 
SetOffThrusterOverride(n, 0f);
}
//FEATURE 0219
