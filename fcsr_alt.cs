...
void Main(string arguments)
{
	...
	else if ("VFTM0".Equals(arguments)) { 
		cuMode=0;
	}
	else if ("VFTM1".Equals(arguments)) { 
		cuMode=1;
	}
	else if ("VFTM2".Equals(arguments)) { 
		cuMode=2;
	}
	debugInfo="cuMode " + cuMode;
...
public void GetBlocks()
{
  ...
  fcsComputer = GridTerminalSystem.GetBlockWithName(FCSComputerNameTag) as IMyProgrammableBlock;
	...
	int.TryParse(meCfg.Get("CURRENT_MODE", cuMode+""), out cuMode);
...
public class RotorBase
{
	...
	public string debugInit = "";
	public string debugInfo;
	public string debugInfoInter;
	public bool isRocket = true;
	public CustomConfiguration cfg;
	public List<float> horis = new List<float>(); // 禁止开火旋转指向1 角度（左右） - 90
	public List<float> horiDs = new List<float>();// 禁止开火旋转范围1 角度 - 45
	public List<float> hori2s = new List<float>();// 禁止开火旋转指向2 角度（后） - 180
	public List<float> horiD2s = new List<float>();// 禁止开火旋转范围2 角度 - 45
	public List<float> verts = new List<float>(); // 禁止开火俯仰的指向1 角度 (左右) - 0
	public List<float> vertDs = new List<float>(); // 禁止开火俯仰下限1 角度 - -30
	public List<float> vert2s = new List<float>(); // 禁止开火俯仰的指向2 角度（后） - 0
	public List<float> vertD2s = new List<float>(); // 禁止开火的俯仰下限2 角度 - 30
	public List<float> offXs = new List<float>(); // 关机时炮台旋转指向
	public List<float> offYs = new List<float>(); // 关机时炮台俯仰指向
	public List<float> onXs = new List<float>(); // 开机时炮台旋转默认
	public List<float> onYs = new List<float>(); // 开机时炮台俯仰默认
	public List<float> ras = new List<float>(); // 优先攻击角度 （基于船）
	public List<float> raDs = new List<float>(); // 不瞄准下限，比这个还低的相似度就不瞄准了，回待机位置
	public List<string> mscNames = new List<string>();
	...
	public RotorBase(IMyBlockGroup thisgroup, MyGridProgram program)
	{
		...
		cfg.Load();
		// TODO
		cfg.Get("verts", ref verts);
		verts = verts.Select(toRa).ToList();
		cfg.Get("vertDs", ref vertDs);
		vertDs = vertDs.Select(toRa).ToList();
		cfg.Get("vert2s", ref vert2s);
		vert2s = vert2s.Select(toRa).ToList();
		cfg.Get("vertD2s", ref vertD2s);
		vertD2s = vertD2s.Select(toRa).ToList();
		cfg.Get("horis", ref horis);
		horis = horis.Select(toRa).ToList();
		cfg.Get("horiDs", ref horiDs);
		horiDs = horiDs.Select(toRa).ToList();
		cfg.Get("hori2s", ref hori2s);
		hori2s = hori2s.Select(toRa).ToList();
		cfg.Get("horiD2s", ref horiD2s);
		horiD2s = horiD2s.Select(toRa).ToList();
		cfg.Get("offXs", ref offXs);
		offXs = offXs.Select(toRa).ToList();
		cfg.Get("offYs", ref offYs);
		offYs = offYs.Select(toRa).ToList();
		cfg.Get("onXs", ref onXs);
	...
	public void Attention(int ATMode, int aeroMode)
	{
		...
		if (this.Weapons.Count == 0 && this.controller != null && remoteBlock!=null && remoteBlock.GetNearestPlayer(out piratePosition)) {
			for(int i = 0; i < this.RotorXs.Count; i ++){
				...
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorXs[i].TargetVelocityRPM = (float)pidXL[i].Filter(-a,2, this.RotorXs[i].TargetVelocityRPM);
			}
			for(int i = 0; i < this.RotorYs.Count; i ++){
			             var a = this.RotorYs[i].Angle;
				if (a > Math.PI) a = a - MathHelper.TwoPi;
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorYs[i].TargetVelocityRPM = (float)pidYL[i].Filter(-a,2, this.RotorYs[i].TargetVelocityRPM);
			}
			return;
		}
		...
		case 0:
			for(int i = 0; i < this.RotorXs.Count; i ++){
			    var a = this.RotorXs[i].Angle - (float)(onXs[cuMode]);
				if (a > Math.PI) a = a - MathHelper.TwoPi;
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorXs[i].TargetVelocityRPM = (float)pidXL[i].Filter(-a,2, this.RotorXs[i].TargetVelocityRPM);
			}
			for(int i = 0; i < this.RotorYs.Count; i ++){
			    var a = this.RotorYs[i].Angle - (float)(onYs[cuMode]);
				if (a > Math.PI) a = a - MathHelper.TwoPi;
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorYs[i].TargetVelocityRPM = (float)pidYL[i].Filter(-a,2, this.RotorYs[i].TargetVelocityRPM);
			}
		...
		case 1:
			...
			for(int i = 0; i < this.RotorXs.Count; i ++){
			            var a = this.RotorXs[i].Angle - xt - (float)(onXs[cuMode]);
				if (a > Math.PI) a = a - MathHelper.TwoPi;
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorXs[i].TargetVelocityRPM = (float)pidXL[i].Filter(-a,2, this.RotorXs[i].TargetVelocityRPM);
			}
			for(int i = 0; i < this.RotorYs.Count; i ++){
			            var a = this.RotorYs[i].Angle - yt - (float)(onYs[cuMode]);
				if (a > Math.PI) a = a - MathHelper.TwoPi;
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorYs[i].TargetVelocityRPM = (float)pidYL[i].Filter(-a,2, this.RotorYs[i].TargetVelocityRPM);
			}
		...
		break;
                        default:
			for(int i = 0; i < this.RotorXs.Count; i ++){
			            var a = this.RotorXs[i].Angle - (float)(offXs[cuMode]);
				if (a > Math.PI) a = a - MathHelper.TwoPi;
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorXs[i].TargetVelocityRPM = (float)pidXL[i].Filter(-a,2, this.RotorXs[i].TargetVelocityRPM);
			}
			for(int i = 0; i < this.RotorYs.Count; i ++){
				var a = this.RotorYs[i].Angle - (float)(offYs[cuMode]);
				if (a > Math.PI) a = a - MathHelper.TwoPi;
				if (a < -Math.PI) a = a + MathHelper.TwoPi;
				this.RotorYs[i].TargetVelocityRPM = (float)pidYL[i].Filter(-a,2, this.RotorYs[i].TargetVelocityRPM);
			}
		...
		Target MyAttackTarget = new Target();
				...
				if (this.MyTarget != null && this.MyTarget.EntityId != 0)
				{
					...
					if (FireTimers.Count() > 0)
					{
						...
						{
							int fire_distance = (int)ShootDistance3;
				...
				{
					this.debugInfo += "\nattention";
					this.Attention(AttentionMode, mode);// debuging
				}
...
//		var aimDirToMe = Vector3D.TransformNormal(aimDir,  refLookAtMatrix);
		...
		if (this.AimBlock.CustomName.Contains("[Arm]")) ea += Math.PI * 0.5;
			...
			bool rx = false;
			bool ry = false;
			rx = angleDeltaAbs(aa, -horis[cuMode]) < horiDs[cuMode]; // aa left +, right -
			rx = rx || angleDeltaAbs(aa + Math.PI, -horis[cuMode]) < horiDs[cuMode];
			bool rx2 = angleDeltaAbs(aa, -hori2s[cuMode]) < horiD2s[cuMode];
                                    bool ry2 = ea - vert2s[cuMode] < -vertD2s[cuMode];
			ry = ea - verts[cuMode] < -vertDs[cuMode];
			//debugInfo += "\nrxry: " + rx + " " + ry;
			//debugInfo += "\nrx2ryx: " + rx2 + " " + ry2;
		...
		aa = (float)onXs[cuMode] * this.RotorXField[0];
		ea = (float)onYs[cuMode] * this.RotorYField[0];
		}
		//debugInfo+= "\nazineed : " + aa;
...
public void Get(string key, ref List<float> res, float def=0, int len=3)
{
string s = Get(key,"");
List<float> tl = s.Split(',').Select(e =>
{
	float v = def;
	float.TryParse(e, out v);
	return v;
}).ToList();
int na = len - tl.Count;
for(int i = 0; i < na; i++) { 
...
public double Filter(double input, int round_d_digits, double co, double step=1, bool debug = false)
{
...
double gain = (gain_p * input) + (gain_i * integral) + (gain_d * derivative);

//gain = MathHelper.Clamp(gain, co - step, co + step);
return gain;
}
