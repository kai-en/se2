...
static double STDBY_Z_OFF = -200;
static int f_no = 0;
double GUIDE_RATE = 0.3;
double ATAN_BASE = 0.1;
double APID_P = 1;
double APID_D = 1.0;

double F_MASS = 1428.8;
double LaunchDist = 300;
...
Random rnd = new Random();
	...
	String gcTargetPanelName="LCD Panel GC Target"+f_no;
        ...
        IMyTerminalBlock radarComputer = null;
        IMyTextSurface radarSurface = null;
        String fcsComputerName="fcs";
        String radarComputerName="radar";
...
List<Vector3D> LTVs = new List<Vector3D>();
	    ...
	    if (TempCollection4.Count > 0)
                ...
                TempCollection4.Clear();
            GridTerminalSystem.GetBlocksOfType<IMyProgrammableBlock>(TempCollection4, a => a.CustomName.Contains(radarComputerName));
            if (TempCollection4.Count > 0)
            { radarComputer = TempCollection4[0];
                radarSurface = ((IMyTextSurfaceProvider)radarComputer).GetSurface(0);
            }
...
Echo(debugInfo);
	    ...
	    Echo("Agile: " + GUIDE_RATE + " " + APID_P + " " + APID_D);
                Echo("\nInfo:\n---------------");
                Echo(Lstrundata);
                ...
                if (gcTargetPanel == null && fcsComputer == null && radarSurface == null)
...
if(radarSurface!=null && targetPanelPosition == Vector3D.Zero) { 
// TODO check radar target list?
}
            FUNNELS.GroupBy(f => f.channel).Select(g => {
                int i = 0;
                g.Select(ThisFunnel => {
	    ...
	    return (x * 1000 + z) * 100 + dis.X;
            ...
            List<FUNNEL> tmpFunnels = new List<FUNNEL>();
		...
		if(TempTurrets.Count>0){
		  // a b K
                        ...
                        NEW_FUNNEL.runningMode = CM_STD;
                        NEW_FUNNEL.IS_CLEAR = false;
                    ...
                    CustomConfiguration cfg = new CustomConfiguration(Key_Gyro);
                    cfg.Load();
                    cfg.Get("CHANNEL", ref NEW_FUNNEL.channel);
                    cfg.Get("MASS", ref NEW_FUNNEL.MissileMass);
                    tmpFunnels.Add(NEW_FUNNEL);
                    }
                }
            // check prefer channel
            var di = "";
            int preferChannel = 0;
            var fgl = FUNNELS.GroupBy(f => f.channel).OrderBy(g => g.Count());
            if (fgl.Any())
            {
                preferChannel = fgl.First().Key;
                if(fgl.Count() == 1)
                {
                    preferChannel = (preferChannel + 1) % 2;
                }
            }
            di += "p" + preferChannel;
            var preferFunnels = tmpFunnels.Where(f => f.channel == preferChannel);
            FUNNEL selected = null;
            if (preferFunnels.Any())
            {
                di += "y";
                selected = preferFunnels.First();
            } else if(tmpFunnels.Any())
            {
                di += "n";
                selected = tmpFunnels.First();
            }
            if (selected != null)
            {
                FUNNELS.Add(selected);
                Lstrundata = di + "Launched Funnel:" + FUNNELS.Count;
                return true;
            }
                return false;
            }
		...
		tmp = Vector3D.Normalize(tmp);
	        ...
	        ENEMY_POS = ENEMY_POS + ThisFunnel.atkOffset;
                ...
                // 散开 按channel
                    ...
                    int posIndi = (idx+2) % 4;
                    if (ThisFunnel.channel != -1)
                {
                    int channel = ThisFunnel.channel;
                    angleIndi = (idx + 1) / 2;
                    posIndi = channel % 2; // 0-left 1-right
                    int ud = (idx + 1) % 2;
                    posIndi += ud * 2;
                }
                    if (posIndi == 0) {
                    ...
                    ENEMY_POS = RC.GetPosition() + dir* STDBY_DISTANCE + RC.WorldMatrix.Backward * STDBY_Z_OFF;
...
//                               debugInfo += "\nidx can use: " + idx + " " + i + "\n"; 
                ...
                if (ThisFunnel.hingeIdx < 0 || ThisFunnel.hingeIdx >= hingeList.Count )
                {
...
// 新算法 end
		...
		foreach (var item in ThisFunnel.WARHEADS){ (item as IMyWarhead).Detonate();}
          ...
          ---------------------------------------     */
            ...
            void GyroTurn6(Vector3D TARGETVECTOR, double GAIN, double DAMPINGGAIN, IMyGyro GYRO, double YawPrev, double PitchPrev, out double NewPitch, out double NewYaw, ref FUNNEL missile, bool isClear, int idx)
            {
                ...
                GYRO.Pitch = (float)MathHelper.Clamp((modAngle(-TRANS_VECT.X)) * GAIN , -60, 60);
		//GYRO.Pitch = (float)MathHelper.Clamp( missile.pidE.Filter(-TRANS_VECT.X, 2) , -30, 30);
                ...
                GYRO.Roll = (float)MathHelper.Clamp((modAngle(-TRANS_VECT.Z)) * GAIN , -60, 60);
